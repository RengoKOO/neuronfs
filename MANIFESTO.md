...
## 🔥 핵심 원리: 텍스트에서 구조로 (Text to Structure)

수십 개의 지시사항을 AI에게 전달할 때, 기존 방식은 하나의 거대한 텍스트에 모든 것을 나열하는 것이다. 
하지만 텍스트는 1차원적이다. 우선순위도 없고, 맥락도 뒤섞이며, 결국 AI는 무거운 규칙들을 무시(Fallback)한다.

NeuronFS는 이 접근을 완전히 뒤집는다. **지시사항을 텍스트 나열이 아닌 파일 시스템의 위상(Topology)으로 표현한다.**

- **폴더의 계층**이 규칙이 적용될 맥락(Context)을 결정한다.
- **폴더의 깊이**가 규칙의 구체성(Specificity)을 제한한다.
- **파일의 이름**이 규칙의 강제력(Weight)을 결정한다.

> **"무엇을 적느냐"가 아니라 "어떤 구조 안에 배치하느냐"가 규칙의 절대성을 결정한다.** 아래의 모든 이야기는 이 원리가 *왜* 작동하는지에 대한 설명이다.
