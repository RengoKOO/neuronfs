...
**1. Letta (MemFS)**
- 방식: 에이전트가 자체적으로 파일을 관리하며 Letta 런타임 서버가 필요.
- 우리 무기: **인프라 종속성 제로(Zero Infrastructure).** 파이썬도, 클라우드 서버도 필요 없이 오직 당신의 OS 권한 하나로 동작.

**2. Engram (인지 메모리 모델)**
- 방식: Ebbinghaus 망각 곡선과 PostgreSQL을 사용하여 인간의 기억 감퇴를 시뮬레이션.
- 우리 무기: **"벡터 DB는 비상 브레이크가 없다."** 우리는 임계치를 3번 어기면 물리적으로 출력을 정지시키는 `bomb.neuron`이 있다. 이것이 진정한 거버넌스다.

**3. Axe (경량 Unix 에이전트)**
- 방식: CLI 파이프라인 기반(`git diff | axe`). 가볍고 매우 빠르다.
- 우리 무기: **"Axe는 빠르지만 나쁜 지시를 내리면 환각을 일으킨다."** 우리의 `brainstem (P0)`은 에이전트의 규칙 기반 환각 자체를 원천 차단한다. P6는 P0를 우회할 수 없다.

**4. Anamnesis / Mem0 (전략적 메모리 엔진)**
- 방식: 권위(Authority)와 관련성(Relevance)에 기반해 무거운 DB를 활용.
- 우리 무기: **"무거운 DB를 쓸 필요가 없다."** 폴더 구조(`mkdir 禁fallback`) 자체가 가장 완벽한 0KB짜리 가중치 연산이다. 디렉토리 트리가 곧 가중치 점수판이다.

|  | .cursorrules | Mem0 | Letta (MemFS) | **NeuronFS** |
|--|-------------|------|----------------|-------------|
