...
NeuronFS는 RAG의 **대체제가 아니다.** 둘은 서로 다른 문제를 푼다.

| | NeuronFS | RAG / Vector DB |
|---|---|---|
| **저장하는 것** | 규칙, 제약, 우선순위 | 문서, 데이터, 사실 |
| **AI에게 주는 것** | "이렇게 행동해라" | "이 정보를 참고해라" |
| **비유** | 헌법 | 백과사전 |
| **우선순위 제어** | 구조적 계층 (결정적) | 유사도 점수 (확률적) |
| **자가 성장** | AI가 `mkdir` → 새 규칙 | 수동 문서 업데이트 |
| **의미 검색** | ❌ 정확한 경로만 | ✅ 유사/퍼지 검색 |
| **규모** | ~500 규칙 (거버넌스 범위) | 10만+ 문서 |
| **비용** | ₩0 | ~$70-200/월 (벡터DB + 임베딩) |

```
┌─────────────────────────────────┐
│  AI Agent                       │
├─────────────────────────────────┤
│  RAG / Vector DB                │  ← "무엇을 아는가" (지식)
├─────────────────────────────────┤
│  NeuronFS                       │  ← "어떻게 행동하는가" (규칙) ★
├─────────────────────────────────┤
│  OS File System                 │
└─────────────────────────────────┘
```

> 함께 쓰면: AI가 **올바른 지식**(RAG)을 **올바른 방식**(NeuronFS)으로 사용한다.
